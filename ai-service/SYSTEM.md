...
| `plan_placements` | **Pro** | Gemini-assisted placement planning with normalized floor polygons, scale hints, and rotation hints. Gemini must return one `PlacementPlan` object shaped as `{ "placements": [...] }`; every `target_polygon` is normalized `0.0-1.0`, never pixel coordinates or nested `placement.polygon`. Floor zones and polygons are practical guidance rather than exact render boxes; deterministic floor placement remains the fallback. |
